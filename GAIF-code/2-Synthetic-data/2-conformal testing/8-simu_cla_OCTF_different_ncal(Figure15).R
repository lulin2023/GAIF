...
Result_RF <- foreach(iter = 1:nr, .combine = "rbind",
                     ...
                     .options.snow = opts)%dopar% {
                       ...
                       for(n_cal in ncal_seq){
                         ...
                         # generate data
                         data <- data_generation_classication1(N=T,mu1= c(1,0,0,0),mu2=c(0,0,-2,-2),p=4,propotion=0.5,pi=0.5)
                         
                         # generate history data and estimate K (diversity threshold)---
                         his_data <- data_generation_classication1(N=n,mu1= c(1,0,0,0),mu2=c(0,0,-2,-2),p=4,propotion=0.5,pi=0.5)
...
head(Result_all)
write.csv(Result_all,"OCTF_ScenarioA_n_500times.csv")
...
Result2.tidy.filtered <- Result2.tidy %>%
  mutate(quant = ifelse(quant > 0.5 & type == "FDR", 0.2, quant))


pdf(file="OCTF_ScenarioA_n.pdf",
    width=12,height=7)
P1 <- ggplot(Result2.tidy.filtered, aes(x = ndraw, y = quant, color=Method)) +
  geom_boxplot(alpha=0.7) +
  scale_x_discrete(name = "n") +
  ylab("") +
