...
result <- foreach(iter = 1:nr, .combine = "rbind", .packages = c("MASS", "onlineFDR","randomForest","caret","nnet",
                  ...
                  .errorhandling = "remove", .options.snow = opts)%dopar% {
                    ...
                    for (pi_1 in pi1_seq) {
                      ...
                      W_cal=Pred(algo,model,X_cal)[,2]  #predict classfication score of calibration data
                      W_test=Pred(algo,model,X_test)[,2]  #predict classfication score of test data
                      ...
                      rej_feedback_LORD=Lord_feedback(pval,alpha,theta,W0=alpha)
                      res <- CiterionCompute(rej_feedback_LORD$R,theta,"LF")
                      res["prop"]=pi_1
                      info=rbind(info,res)
                      
                      
                      rej_feedback_LORD_conse=Lord_feedback_conservative(pval,alpha,theta,W0=alpha)
                      ...
                      rej_feedback_SA=SAFFRON_feedback(pval,alpha=alpha,theta=theta,w0=alpha,lambda=0.5)
                      res <- CiterionCompute(rej_feedback_SA$R,theta,"SF")
                      res["prop"]=pi_1
                      info=rbind(info,res)
                      
                      rej_feedback_SA_conse=SAFFRON_feedback_conservative(pval,alpha,theta,w0=alpha,lambda=0.5)
...
cla_plots_vary_prop <- ggarrange(P1, P2, ncol=2, nrow=1, common.legend = TRUE, legend="bottom",
                                 font.label = list(size = 20, face = "bold"))

pdf(file = "cla_plots_vary_prop.pdf",width = 10,height = 4) 
cla_plots_vary_prop
dev.off()
