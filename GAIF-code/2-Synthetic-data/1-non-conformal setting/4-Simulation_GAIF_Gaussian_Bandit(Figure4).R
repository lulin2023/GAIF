...
source(file.path(base, "useful functions", "algoclass_OnSel.R"))
...
result <- foreach(iter = 1:nr, .combine = "rbind", .packages = c("MASS", "onlineFDR"), .errorhandling = "remove", .options.snow = opts)%dopar% {
  ...
  for (pi_1 in pi1_seq) {
    ...
    rej_feedback_SA=SAFFRON_feedback_bandit(data$p,alpha=alpha,w0=alpha/2,theta=data$theta,lambda=0.5)
    res=CiterionCompute(rej_feedback_SA$R,data$theta,"SF-BI")
    res["prop"]=pi_1
    info=rbind(info,res)
    
    
    rej_feedback_LORD=Lord_feedback_bandit(data$p,alpha,data$theta,W0=alpha/2)
    res=CiterionCompute(rej_feedback_LORD$R,data$theta,"LF-BI")
    res["prop"]=pi_1
    info=rbind(info,res)
...
pp$Method=factor(pp$Method,levels = c("SF-BI","LF-BI","SAFFRON","LORD++","LOND"))
...
GAIF_Gaussian_Bandit <- ggarrange(P1, P2, ncol=2, nrow=1, common.legend = TRUE, legend="bottom",
                           font.label = list(size = 20, face = "bold"))

pdf(file = "GAIF_Gaussian_Bandit.pdf",width = 10,height = 4) 
GAIF_Gaussian_Bandit
dev.off()
dev.new()
